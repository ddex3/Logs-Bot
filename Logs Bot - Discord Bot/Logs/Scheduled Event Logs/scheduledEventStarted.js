...
module.exports = (client, config) => {
  ...
  client.on('guildScheduledEventUpdate', async (oldEvent, newEvent) => {
    ...
    if (oldEvent.status !== newEvent.status && newEvent.status === 2) {
      ...
      db.get(
        "SELECT channel_id FROM logs_channels WHERE guild_id = ? AND log = 'Scheduled Event Started'",
        [guild.id],
        async (_, row) => {
          if (!row) return
          ...
          const embed = new EmbedBuilder()
            .setTitle('Scheduled Event Started')
            .addFields(
              ...
              { name: 'Action', value: 'Scheduled Event Started', inline: true },
          ...
          logChannel.send({ embeds: [embed] }).then(sent => {
            ...
            saveServerLog(guild.id, 'event', 'Scheduled Event Started', 0x67e68d, fields, row.channel_id, sent.id, guild.iconURL(), guild.name)
          }).catch(() => {})
        }
      )
    }
