...
module.exports = (client, config) => {
  ...
  client.on('guildScheduledEventUserRemove', async (event, user) => {
    ...
    db.get(
      "SELECT channel_id FROM logs_channels WHERE guild_id = ? AND log = 'Event Subscription Removed'",
      [guild.id],
      async (_, row) => {
        if (!row) return
        ...
        const embed = new EmbedBuilder()
          .setTitle('User Unsubscribed from Event')
          .addFields(
            { name: 'Event', value: `${event.name}`, inline: true },
            { name: 'User', value: `<@${user.id}> (${user.tag})`, inline: true },
            { name: 'Action', value: 'Event Subscription Removed', inline: true },
        ...
        logChannel.send({ embeds: [embed] }).then(sent => {
          ...
          saveServerLog(guild.id, 'event', 'Event Subscription Removed', 0xf25252, fields, row.channel_id, sent.id, guild.iconURL(), guild.name)
        }).catch(() => {})
      }
    )
  })
