...
module.exports = (client, config) => {
  ...
  client.on('roleCreate', async role => {
    ...
    db.get(
      "SELECT channel_id FROM logs_channels WHERE guild_id = ? AND log = 'Role Created'",
      [guild.id],
      async (_, row) => {
        if (!row) return
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: 'Online Display', value: capitalize(role.hoist), inline: true },
            { name: 'Mentionable', value: capitalize(role.mentionable), inline: true },
            { name: 'Action', value: 'Role Created', inline: true },
        ...
        logChannel.send({ embeds: [embed] }).then(sent => {
          ...
          saveServerLog(guild.id, 'role', 'Role Created', 0x57F287, fields, row.channel_id, sent.id, guild.iconURL(), guild.name)
        }).catch(() => {})
      }
    )
  })
