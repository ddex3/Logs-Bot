...
module.exports = (client, config) => {
  ...
  client.on('voiceStateUpdate', (oldState, newState) => {
    ...
    db.get("SELECT channel_id FROM logs_channels WHERE guild_id = ? AND log = 'Stopped Video'",
      [guild.id], (_, row) => {
        if (!row) return
        ...
        const embed = new EmbedBuilder()
          ...
          .setFooter({ text: `${guild.client.user.username} • Stopped Video`, iconURL: guild.client.user.displayAvatarURL() })
          .setTimestamp()
        ...
        logChannel.send({ embeds: [embed] }).then(sent => {
          ...
          saveServerLog(guild.id, 'voice', 'Stopped Video', 0xf25252, fields, row.channel_id, sent.id, guild.iconURL(), `${guild.client.user.username} • Stopped Video`)
        }).catch(() => {})
    })
  })
}
