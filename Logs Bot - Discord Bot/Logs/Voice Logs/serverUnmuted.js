...
module.exports = (client, config) => {
  ...
  client.on('voiceStateUpdate', async (oldState, newState) => {
    ...
    db.get(
      "SELECT channel_id FROM logs_channels WHERE guild_id = ? AND log = 'Server Unmuted'",
      [guild.id],
      (_, row) => {
        if (!row) return
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: 'Unmuted By', value: executor, inline: false },
            { name: 'Time', value: `<t:${timestamp}> (<t:${timestamp}:R>)`, inline: false }
          )
          .setColor('#67e68d')
          .setThumbnail(guild.iconURL())
          .setFooter({ text: `${guild.client.user.username} • Server Unmuted`, iconURL: guild.client.user.displayAvatarURL() })
          .setTimestamp()
        ...
        logChannel.send({ embeds: [embed] }).then(sent => {
          ...
          saveServerLog(guild.id, 'voice', 'Server Unmuted', 0x67e68d, fields, row.channel_id, sent.id, guild.iconURL(), `${guild.client.user.username} • Server Unmuted`)
        }).catch(() => {})
      }
    )
  })
