...
module.exports = (client, config) => {
  ...
  client.on('voiceStateUpdate', async (oldState, newState) => {
    ...
    db.get(
      "SELECT channel_id FROM logs_channels WHERE guild_id = ? AND log = 'Server Muted'",
      [guild.id],
      (_, row) => {
        if (!row) return
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: 'Muted By', value: executor, inline: false },
            { name: 'Time', value: `<t:${timestamp}> (<t:${timestamp}:R>)`, inline: false }
          )
          .setColor('#f25252')
          .setThumbnail(guild.iconURL())
          .setFooter({ text: `${guild.client.user.username} • Server Muted`, iconURL: guild.client.user.displayAvatarURL() })
          .setTimestamp()
        ...
        logChannel.send({ embeds: [embed] }).then(sent => {
          ...
          saveServerLog(guild.id, 'voice', 'Server Muted', 0xf25252, fields, row.channel_id, sent.id, guild.iconURL(), `${guild.client.user.username} • Server Muted`)
        }).catch(() => {})
      }
    )
  })
