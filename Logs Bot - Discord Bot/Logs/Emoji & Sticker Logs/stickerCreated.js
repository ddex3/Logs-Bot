...
module.exports = (client, config) => {
  ...
  client.on('stickerCreate', async sticker => {
    ...
    const logs = await guild.fetchAuditLogs({ type: AuditLogEvent.StickerCreate, limit: 1 })
    ...
    db.get(
      "SELECT channel_id FROM logs_channels WHERE guild_id = ? AND log = 'Sticker Created'",
      [guild.id],
      async (_, row) => {
        if (!row) return
        ...
        const embed = new EmbedBuilder()
          ...
          .setFooter({ text: `${guild.client.user.username} • Sticker Created`, iconURL: guild.client.user.displayAvatarURL() })
          .setTimestamp()
        ...
        logChannel.send({ embeds: [embed] }).then(sent => {
          ...
          saveServerLog(guild.id, 'sticker', 'Sticker Created', 0x67e68d, fields, row.channel_id, sent.id, guild.iconURL(), `${guild.client.user.username} • Sticker Created`)
        }).catch(() => {})
      }
    )
  })
