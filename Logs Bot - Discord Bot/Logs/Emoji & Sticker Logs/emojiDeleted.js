...
module.exports = (client, config) => {
  ...
  client.on('emojiDelete', async emoji => {
    ...
    db.get(
      "SELECT channel_id FROM logs_channels WHERE guild_id = ? AND log = 'Emoji Deleted'",
      [guild.id],
      async (_, row) => {
        if (!row) return
        ...
        const embed = new EmbedBuilder()
          ...
          .setFooter({ text: `${guild.client.user.username} • Emoji Deleted`, iconURL: guild.client.user.displayAvatarURL() })
          .setTimestamp()
        ...
        logChannel.send({ embeds: [embed] }).then(sent => {
          ...
          saveServerLog(guild.id, 'emoji', 'Emoji Deleted', 0xf25252, fields, row.channel_id, sent.id, guild.iconURL(), `${guild.client.user.username} • Emoji Deleted`)
        }).catch(() => {})
      }
    )
  })
